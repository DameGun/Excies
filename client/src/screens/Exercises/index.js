...
export default function ExercisesModalScreen({ route, navigation }) {
    ...
    function handleExerciseButton(item) {
        if (item.exercise_id) {
            dispatch(thunkDeleteExerciseListItem({ payload: { list_id, username, list_item_id: item.id } }))
        }
        else {
            dispatch(thunkCreateExerciseListItem({ payload: { list_id, username, exercise_id: item.id } }));
        }
    }
